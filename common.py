...
class Config:
    ...
    def initialize(self, args):
        ...
        if self.name == 'view':
            self.mot_en_channels = [self.len_joints + 2, 64, 96, 160]
            self.view_en_channels = [self.len_joints, 64, 96, 128, 32]
            self.de_channels = [self.mot_en_channels[-1], 128, 64, self.len_joints + 2]
    ...
    def initializecom(self):
        self.name = 'view' 
        self.use_triplet = None 
        self.use_footvel_loss = None 

        os.environ["CUDA_VISIBLE_DEVICES"] = '0' #str(args.gpu_ids)
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
