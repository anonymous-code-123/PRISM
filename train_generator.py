...
from model.builder import get_autoencoder
    ...
    for e in range(config.nr_epochs):
        ...
        for b, data in enumerate(pbar):
            ...
            if clock.step % config.val_frequency == 0:
                ...
                # ==== PVR 验证 ====
                seq_hat = outputs['recon_seq']
                pvr_dict = compute_pvr(seq_hat)

                for k, v in pvr_dict.items():
                    val_tb.add_scalar(f'val/{k}', v, clock.step)

                print(f"[PVR] Bone: {pvr_dict['PVR_bone']:.1%} | Joint: {pvr_dict['PVR_joint']:.1%} | Acc: {pvr_dict['PVR_acc']:.1%}")
