...
def gendata(data_path, out_path, split_path, benchmark='xview', part='eval'):
    ...
    for i, s in enumerate(tqdm(sample_name[:])):
        vid = s[:-5]
        data, nb_frame = read_xyz(os.path.join(data_path, s), max_body = max_body, num_joint = num_joint)
        nb_split = math.ceil(nb_frame/wd)
        fp = np.zeros((nb_split, 2, wd+1, num_joint, max_body_true), dtype=np.float32)
        labelp = np.zeros((nb_split, wd, num_classes), np.float32)
        ...
        for f in range(nb_split):
            ...
            fp[f, :, 0:min(wd, data.shape[1]-wd*f), :, :] = data[:,wd*f:min(wd*f+wd, data.shape[1]),:,:]
            fp[f, -1, -1, -1, -1] = min(wd, data.shape[1]-wd*f)
            labelp[f, 0:min(wd, data.shape[1]-wd*f), :] = label[wd*f:min(wd*f+wd, data.shape[1]),:]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Charades Data Converter.')
    parser.add_argument('--data_path', default='/data/stars/user/rdai/PhD_work/cvpr2020/Charades_v1/Charades_v1_lcrnet/')
    parser.add_argument('--split_path',
                        default='../data/charades.json')
    parser.add_argument('--out_folder', default='../data/charades64+/')
