...
class _MixamoDatasetBase(Dataset):
    ...
    def preprocessing(self, item, view_angle=None, param=None):
        ...
        # interpolation to 17 joints
     #   motion3d = np.concatenate((motion3d, (motion3d[0:1, :, :] + motion3d[1:2, :, :])/2, (motion3d[1:2, :, :] + motion3d[8:9, :, :])/2), axis=0)
...
def get_meanpose(config):
    ...
    # 15,2 to 17,2
    #meanpose = np.concatenate((meanpose, (meanpose[0:1, :] + meanpose[1:2, :])/2, (meanpose[1:2, :] + meanpose[8:9, :])/2), axis=0)
    #stdpose = np.concatenate((stdpose, (stdpose[0:1, :] + stdpose[1:2, :])/2, (stdpose[1:2, :] + stdpose[8:9, :])/2), axis=0)
    
    #not used
    #meanpose = np.concatenate((meanpose, np.zeros((2, 2))), axis=0)
    #stdpose = np.concatenate((stdpose, np.zeros((2, 2))), axis=0)
